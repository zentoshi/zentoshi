...
bool CPrivateSendQueue::Sign()
{
    ...
    if (deterministicMNManager->IsDeterministicMNsSporkActive()) {
        ...
        sig.GetBuf(vchSig);
    } else if (sporkManager.IsSporkActive(Spork::SPORK_6_NEW_SIGS)) {
        uint256 hash = GetSignatureHash();
...
bool CPrivateSend::IsCollateralValid(const CTransaction& txCollateral)
{
...

    {
        ...
        if(!AcceptToMemoryPool(mempool, validationState, MakeTransactionRef(txCollateral), nullptr, nullptr, false, true, true)) {
