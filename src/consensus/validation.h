...
{
    return ::GetSerializeSize(block) * (WITNESS_SCALE_FACTOR - 1) + ::GetSerializeSize(block);
}
static inline int64_t GetTransactionInputWeight(const CTxIn& txin)
{
    // scriptWitness size is added here because witnesses and txins are split up in segwit serialization.
    return ::GetSerializeSize(txin) * (WITNESS_SCALE_FACTOR - 1) + ::GetSerializeSize(txin) + ::GetSerializeSize(txin.scriptWitness.stack);
}
