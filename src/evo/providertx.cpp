...
bool CheckProRegTx(const CTransaction& tx, const CBlockIndex* pindexPrev, CValidationState& state)
{
    ...
    if (ptx.keyIDOwner.IsNull() || !ptx.pubKeyOperator.IsValid() || ptx.keyIDVoting.IsNull()) {
        return state.Invalid(ValidationInvalidReason::PROTX_BAD, false, REJECT_INVALID, "bad-protx-key-null");
    }
    if (!ptx.scriptPayout.IsPayToPublicKeyHash() && !ptx.scriptPayout.IsPayToScriptHash()) {
        return state.Invalid(ValidationInvalidReason::PROTX_BAD, false, REJECT_INVALID, "bad-protx-payee");
    }

    CTxDestination payoutDest;
    if (!ExtractDestination(ptx.scriptPayout, payoutDest)) {
        // should not happen as we checked script types before
        return state.Invalid(ValidationInvalidReason::PROTX_BAD, false, REJECT_INVALID, "bad-protx-payee-dest");
    }
    ...
    if (ptx.nOperatorReward > 10000) {
        return state.Invalid(ValidationInvalidReason::PROTX_BAD, false, REJECT_INVALID, "bad-protx-operator-reward");
    }
    ...
    if (!ptx.collateralOutpoint.hash.IsNull()) {
        Coin coin;
        if (!GetUTXOCoin(ptx.collateralOutpoint, coin) || coin.out.nValue != Params().GetConsensus().MasternodeCollateral()) {
            return state.Invalid(ValidationInvalidReason::PROTX_BAD, false, REJECT_INVALID, "bad-protx-collateral");
        }

        if (!ExtractDestination(coin.out.scriptPubKey, collateralTxDest)) {
            return state.Invalid(ValidationInvalidReason::PROTX_BAD, false, REJECT_INVALID, "bad-protx-collateral-dest");
        }
        ...
        if (keyForPayloadSig.IsNull()) {
            return state.Invalid(ValidationInvalidReason::PROTX_BAD, false, REJECT_INVALID, "bad-protx-collateral-pkh");
        }

        collateralOutpoint = ptx.collateralOutpoint;
    } else {
        if (ptx.collateralOutpoint.n >= tx.vout.size()) {
            return state.Invalid(ValidationInvalidReason::PROTX_BAD, false, REJECT_INVALID, "bad-protx-collateral-index");
        }
        if (tx.vout[ptx.collateralOutpoint.n].nValue != Params().GetConsensus().MasternodeCollateral()) {
            return state.Invalid(ValidationInvalidReason::PROTX_BAD, false, REJECT_INVALID, "bad-protx-collateral");
        }
    ...
    if (collateralTxDest == CTxDestination(PKHash(ptx.keyIDOwner)) || collateralTxDest == CTxDestination(PKHash(ptx.keyIDVoting))) {
        return state.Invalid(ValidationInvalidReason::PROTX_BAD, false, REJECT_INVALID, "bad-protx-collateral-reuse");
    }
    ...
    if (pindexPrev) {
        ...
        if (mnList.HasUniqueProperty(ptx.addr) && mnList.GetUniquePropertyMN(ptx.addr)->collateralOutpoint != collateralOutpoint) {
            return state.Invalid(ValidationInvalidReason::PROTX_BAD, false, REJECT_DUPLICATE, "bad-protx-dup-addr");
        }
    ...
    if (pindexPrev) {
        ...
        if (ptx.scriptOperatorPayout != CScript()) {
            if (mn->nOperatorReward == 0) {
                ...
                return state.Invalid(ValidationInvalidReason::PROTX_BAD, false, REJECT_INVALID, "bad-protx-operator-payee");
            }
...
bool CheckProUpRegTx(const CTransaction& tx, const CBlockIndex* pindexPrev, CValidationState& state)
{
    ...
    if (pindexPrev) {
        ...
        if (payoutDest == CTxDestination(PKHash(dmn->pdmnState->keyIDOwner)) || payoutDest == CTxDestination(PKHash(ptx.keyIDVoting))) {
            return state.Invalid(ValidationInvalidReason::PROTX_BAD, false, REJECT_INVALID, "bad-protx-payee-reuse");
        }
