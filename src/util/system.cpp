...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Zentoshi
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Zentoshi
    // Mac: ~/Library/Application Support/Zentoshi
    // Unix: ~/.zentoshi
#ifdef WIN32
    // Windows
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = gArgs.GetArg("-conf", BITCOIN_CONF_FILENAME);
    ...
    if (stream.good()) {
        ...
        if (emptyIncludeConf) {
            ...
            includeconf = gArgs.GetArgs("-includeconf");
            {
                std::vector<std::string> includeconf_net(GetArgs(std::string("-") + chain_id + ".includeconf"));
                includeconf.insert(includeconf.end(), includeconf_net.begin(), includeconf_net.end());
                ...
                if (chain_id_final != chain_id) {
                    ...
                    includeconf_net = gArgs.GetArgs(std::string("-") + chain_id_final + ".includeconf");
                    includeconf.insert(includeconf.end(), includeconf_net.begin(), includeconf_net.end());
                }
            }
            for (const std::string& to_include : includeconf) {
