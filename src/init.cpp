...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-masternodeblsprivkey=<n>", "Set the masternode BLS private key", ArgsManager::ALLOW_ANY, OptionsCategory::MASTERNODE);
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    util::ThreadRename("zentoshi-loadblk");
...
bool AppInitParameterInteraction()
{
    ...
    // if using block pruning, then exit
    if (gArgs.GetArg("-prune", 0)) {
        return InitError(_("Zentoshi does not support pruning.").translated);
    }
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    privateSendClient.fPrivateSendRunning = GetMainWallet()->IsLocked() ? false : gArgs.GetBoolArg("-privatesendautostart", DEFAULT_PRIVATESEND_AUTOSTART);
