...
public:
    ...
    // ZenX Specific WalletInitInterface InitPrivateSendSettings
 ...
 * between zenxd, and zenx-qt and non-server tools.
*/
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    if (mode == HMM_BITCOIN_QT)
        strUsage += HelpMessageOpt("-windowtitle=<name>", _("Sets a window title which is appended to \"Zentoshi Core - \""));
    ...
    strUsage += HelpMessageOpt("-sporkaddr=<zenxaddress>", strprintf(_("Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.")));
...
std::string LicenseInfo()
{
    ...
    const std::string URL_WEBSITE = "<https://zenx.org>";
...
/** Sanity checks
 *  Ensure that Zentoshi Core is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Zentoshi Core version %s\n", FormatFullVersion());
}
...
{
    // Make sure only a single Zentoshi Core process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if Zentoshi Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if Zentoshi Core is started while in a temporary directory.\n",
    ...
    // ********************************************************* Step 10c: schedule ZenX-specific tasks
