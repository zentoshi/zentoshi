...
const char * const BITCOIN_CONF_FILENAME = "zenx.conf";
const char * const BITCOIN_PID_FILENAME = "zenxd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "zenxd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ZenXCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ZenXCore
    // Mac: ~/Library/Application Support/ZenXCore
    // Unix: ~/.zenxcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ZenXCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ZenXCore";
#else
    // Unix
    return pathRet / ".zenxcore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty zenx.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Zentoshi Core") == std::string::npos) {
