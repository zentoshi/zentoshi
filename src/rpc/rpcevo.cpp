...
std::string GetHelpString(int nParamNum, std::string strParamName)
{
    static const std::map<std::string, std::string> mapParamHelp = {
        {"collateralAddress",
            "%d. \"collateralAddress\"        (string, required) The zentoshi address to send the collateral to.\n"
        },
        {"collateralHash",
        ...
        {"ownerAddress",
            "%d. \"ownerAddress\"             (string, required) The zentoshi address to use for payee updates and proposal voting.\n"
...
static void FundSpecialTx(CWallet* pwallet, CMutableTransaction& tx, const SpecialTxPayload& payload, const CTxDestination& fundDest)
{
    ...
    auto nTxRef = MakeTransactionRef(std::move(wtx));

    if (!pwallet->CreateTransaction(vecSend, nTxRef, reservekey, nFee, nChangePos, strFailReason, coinControl, false, ALL_COINS, false, tx.vExtraPayload.size())) {
...
UniValue protx_register(const JSONRPCRequest& request)
{
    ...
    CAmount collateralAmount = Params().CollateralAmount();
