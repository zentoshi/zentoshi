    ...
    for (auto& pnode : vNodesCopy)
    {
        ...
        // NORMAL NETWORK MODE - TESTNET/MAINNET
        {
            ...
            if(nCurrentAsset == MASTERNODE_SYNC_LIST) {
                ...
                LogPrint(BCLog::MASTERNODE, "CMasternodeSync::ProcessTick -- nTick %d nCurrentAsset %d nTimeLastBumped %lld GetTime() %lld diff %lld\n", nTick, nCurrentAsset, nTimeLastBumped, GetTime(), GetTime() - nTimeLastBumped);
            ...
            if(nCurrentAsset == MASTERNODE_SYNC_MNW) {
                ...
                LogPrint(BCLog::MNPAYMENTS, "CMasternodeSync::ProcessTick -- nTick %d nCurrentAsset %d nTimeLastBumped %lld GetTime() %lld diff %lld\n", nTick, nCurrentAsset, nTimeLastBumped, GetTime(), GetTime() - nTimeLastBumped);
                // check for timeout first
