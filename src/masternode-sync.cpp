...
void CMasternodeSync::SwitchToNextAsset(CConnman& connman)
{
    switch(nCurrentAsset)
    {
        ...
        case(MASTERNODE_SYNC_GOVERNANCE):
            ...
            LogPrint(BCLog::MASTERNODE, "CMasternodeSync::SwitchToNextAsset -- Sync has finished\n");
...
void CMasternodeSync::ProcessTick(CConnman& connman)
{
    ...
    if(GetTime() - nTimeLastProcess > 60*60) {
        LogPrint(BCLog::MASTERNODE, "CMasternodeSync::ProcessTick -- WARNING: no actions for too long, restarting sync...\n");
        Reset();
    ...
    if(IsFailed()) {
        if(nTimeLastFailure + (1*60) < GetTime()) { // 1 minute cooldown after failed sync
            LogPrint(BCLog::MASTERNODE, "CMasternodeSync::ProcessTick -- WARNING: failed to sync, trying again...\n");
            Reset();
            SwitchToNextAsset(connman);
        }
        return;
    ...
    LogPrint(BCLog::MASTERNODE, "CMasternodeSync::ProcessTick -- nTick %d nCurrentAsset %d nTriedPeerCount %d nSyncProgress %f\n", nTick, nCurrentAsset, nTriedPeerCount, nSyncProgress);
    ...
    for (auto& pnode : vNodesCopy)
    {
        ...
        // NORMAL NETWORK MODE - TESTNET/MAINNET
        {
            ...
            if(!netfulfilledman.HasFulfilledRequest(pnode->addr, "spork-sync")) {
                ...
                LogPrint(BCLog::MASTERNODE, "CMasternodeSync::ProcessTick -- nTick %d nCurrentAsset %d -- requesting sporks from peer=%d\n", nTick, nCurrentAsset, pnode->GetId());
            }
            ...
            if(nCurrentAsset == MASTERNODE_SYNC_GOVERNANCE) {
                ...
                if(GetTime() - nTimeLastBumped > MASTERNODE_SYNC_TIMEOUT_SECONDS) {
                    LogPrint(BCLog::MASTERNODE, "CMasternodeSync::ProcessTick -- nTick %d nCurrentAsset %d -- timeout\n", nTick, nCurrentAsset);
                    if(nTriedPeerCount == 0) {
                        LogPrint(BCLog::MASTERNODE, "CMasternodeSync::ProcessTick -- WARNING: failed to sync %s\n", GetAssetName());
                ...
                if(netfulfilledman.HasFulfilledRequest(pnode->addr, "governance-sync")) {
                    ...
                    if(nObjsLeftToAsk == 0) {
                        ...
                        if(GetTime() - nTimeNoObjectsLeft > MASTERNODE_SYNC_TIMEOUT_SECONDS &&
                            ...
                            LogPrint(BCLog::MASTERNODE, "CMasternodeSync::ProcessTick -- nTick %d nCurrentAsset %d -- asked for all objects, nothing to do\n", nTick, nCurrentAsset);
