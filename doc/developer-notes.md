...
Zentoshi Core is a multithreaded application, and deadlocks or other multithreading bugs
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_zenx
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_zenx --log_level=test_suite
$ valgrind -v --leak-check=full src/zenxd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_zenx.coverage/index.html`.
```
...
However, in open source software such as Zentoshi Core, where everyone uses
...
pay attention to for reviewers of Zentoshi Core code.

General Zentoshi Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Zentoshi Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Zentoshi Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `zenx-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `zenx-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Zentoshi Core, the wallet was always
