...
*Setup instructions for a Gitian build of Zentoshi Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the ZenX
...
- [Building Zentoshi Core](#building-zenx-core)
...
Clone the git repositories for Zentoshi Core and Gitian.
...
in the Zentoshi Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Zentoshi Core
----------------

To build Zentoshi Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the Zentoshi Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/zenx/.git/
...
./bin/gbuild --commit zenx=${COMMIT} --url zenx=${URL} ../zenx/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit zenx=${COMMIT} --url zenx=${URL} ../zenx/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit zenx=${COMMIT} --url zenx=${URL} ../zenx/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the zenx git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../zenx/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/zenx
SIGPATH=/some/root/path/zenx-detached-sigs

./bin/gbuild --url zenx=${BTCPATH},signature=${SIGPATH} ../zenx/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/zenx-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/zenx-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/zenx-osx-build.assert
```
