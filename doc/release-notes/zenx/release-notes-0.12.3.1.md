Zentoshi Core version 0.12.3.1
==========================
...
Because release 0.12.2.2 included the [per-UTXO fix](release-notes/zenx/release-notes-0.12.2.2.md#per-utxo-fix)
...
We introduced a new signature format for ZenX-specific network messages,
...
However this option is only available in so called Lite Mode. In this mode, ZenX specific features are disabled, meaning
...
- `debug` rpc now requires categories to be separated via `+`, not `,` like before (e.g. `zenx+net`);
...
- introduced in Zentoshi Core 0.12.3.1: `allowprivatenet`, `bip9params`, `sporkaddr`, `devnet`;
- backported from Bitcoin Core 0.13/0.14: `blockreconstructionextratxn`, `maxtimeadjustment`, `maxtipage`,
`incrementalrelayfee`, `dustrelayfee`, `blockmintxfee`.

See `Help -> Command-line options` in Qt wallet or `zenxd --help` for more info.
...
mnprivkey during MN setup and NOT the one that controls the 1000 ZENX collateral) which should make the process of pairing with
...
We now use a recent masternode list to feed the hardcoded seed nodes list in Zentoshi Core. This list was previously
...
- [`08033ffe4`](https://github.com/dashpay/dash/commit/08033ffe4) Reject ZenX-specific messages from obsolete peers (#1983)
...
- [`532b9fa3d`](https://github.com/dashpay/dash/commit/532b9fa3d) Use OpenNetworkConnection instead of calling ConnectNode directly in ZenX code (#1857)
...
- [`4ea790377`](https://github.com/dashpay/dash/commit/4ea790377) ZenXify lib names (#2035)
...
- [`c65613350`](https://github.com/dashpay/dash/commit/c65613350) Switch masternode id in ZenX data structures from CTxIn to COutPoint (#1933)
...
- [`a7fa07a30`](https://github.com/dashpay/dash/commit/a7fa07a30) Drop BOOST_FOREACH and use references in loops (const ref where applicable, ZenX code only) (#1899)
...
ZenX was previously known as Darkcoin.
