...
RUN cd zenx_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} zenx
RUN useradd -u ${USER_ID} -g zenx -s /bin/bash -m -d /zenx zenx
...
RUN mkdir /zenx-src && \
  ...
  chown $USER_ID:$GROUP_ID /zenx-src && \
